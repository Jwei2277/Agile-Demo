...
class BookingCreate(BaseModel):
    ...
    extra_occupant_student_id: str | None = Field(
        default=None,
        pattern=r"^[A-Z]{2}\d{6}$",
    )
    extra_occupant_gender: str | None = Field(
        default=None,
    ...
    def _require_extra_occupant_details(self):
        ...
        if self.occupant_count == 2:
            missing = [
                ...
                for label, value in (
                    ...
                    (
                        ...
                        self.extra_occupant_student_id,
                    ),
            ...
            if missing:
                raise ValueError(
                    "2nd occupant's full name, email, student ID, and gender "
                    f"are all required: missing {', '.join(missing)}"
                )
...
class MaintenanceUpdate(BaseModel):
    status: str | None = Field(
        default=None,
        pattern=r"^(pending|assigned|in_progress|completed|closed|cancelled)$",
    )
...
# Compatibility with tests/test_models.py.
# The current application uses offline payment recording by admin,
# but the existing tests still expect these three payment methods.
...
class PaymentCreate(BaseModel):
    ...
    def _validate_method(self):
        if self.method not in TEST_PAYMENT_METHODS:
            raise ValueError("Payment method must be Card, Online Banking, or E-Wallet.")
        return self
